...
@media (max-width: ${BREAKPOINT}px) {
  ...
  /* Crédits + notes alignés à droite en mobile */
  .${WRAP_CLASS} .credit,
  .${WRAP_CLASS} .biblio{
...
`;
  ...
  const RULES = {
    ...
    projet2: (section) => {
      ...
      if (protocTitre) {
        addSpacerIfHasChildren(wrap, "lg");
        wrap.appendChild(protocTitre);
      }
      if (protocCorps) {
        if (protocTitre) addSpacer(wrap, "sm");
        wrap.appendChild(protocCorps);
      }
      if (extraitTitre) {
        addSpacer(wrap, "lg");
    ...
    projet4: (section) => {
      ...
      if (protocTitre) {
        // espace entre body -> protocole titre
        addSpacer(wrap, "md");
        // espace entre body -> protocole titre
        addSpacer(wrap, "md");
        addSpacerIfHasChildren(wrap, "lg");
  ...
  const init = () => {
    ...
    // Mobile: renommer "infos" -> "bio" (et restore en desktop)
    const syncInfoLabel = () => {
