...
(() => {
  ...
  function ensureStyle() {
    ...
    style.textContent = `
      ...
      body.${MOBILE_CLASS} section[id^="projet"] .m-mobile-wrap{
        width: 100%;
      }
      ...
      /* credit x3 (applied only on cloned nodes we tag) */
      body.${MOBILE_CLASS} .m-credit-3x{ font-size: 3em !important; }
  ...
  function buildMobileFor(section) {
    ...
    switch (id) {
      case "projet1": { // Liaisons végétales
        nodesToAppend.push(cloneNodeKeep(pickOne(".extrait-titre")));
        pickAll(".extrait").forEach((n) => nodesToAppend.push(cloneNodeKeep(n)));
        const credit = cloneNodeKeep(pickOne(".credit"));
        if (credit) credit.classList.add("m-credit-3x");
        nodesToAppend.push(credit);
      ...
      case "projet2": { // Panser
        ...
        nodesToAppend.push(protocoleTitle);
        nodesToAppend.push(cloneNodeKeep(pickOne(".protocole-corps")));
        nodesToAppend.push(cloneNodeKeep(extraitTitle));
        nodesToAppend.push(cloneNodeKeep(pickOne(".extrait")));
        const credit = cloneNodeKeep(pickOne(".credit"));
        if (credit) credit.classList.add("m-credit-3x");
        nodesToAppend.push(credit);
      ...
      case "projet3": { // Aux mornes
        const extraitTitleLike = pickOne(".intro") || pickOne(".extrait-titre");
        const fsTitle = computedFontSizePx(extraitTitleLike);

        const intro = cloneNodeKeep(pickOne(".intro"));
        if (intro && fsTitle) intro.style.fontSize = fsTitle;
        nodesToAppend.push(intro);

        // Body = all context blocks
        pickAll(".context.autre-corps").forEach((n) => nodesToAppend.push(cloneNodeKeep(n)));

        // Notes = credits/biblio scaled x3
        ...
        if (notes.childNodes.length) {
          notes.classList.add("m-credit-3x");
          nodesToAppend.push(notes);
        }
      ...
      case "projet4": { // Caisse de résonance
        ...
        pickAll(".context.context-p4").forEach((n) => nodesToAppend.push(cloneNodeKeep(n)));
      ...
      case "projet5": { // Gestes possibles
        const introEl = pickOne(".intro");
        const fsTitle = computedFontSizePx(introEl || pickOne(".extrait-titre"));

        const intro = cloneNodeKeep(introEl);
        if (intro && fsTitle) intro.style.fontSize = fsTitle;
        nodesToAppend.push(intro);

        nodesToAppend.push(cloneNodeKeep(pickOne(".context.context-p5")));
      ...
      case "projet6": { // Les marges forcées
        ...
        // Body: main context (exclude links)
        nodesToAppend.push(cloneNodeKeep(pickOne(".context.context-p6")));

        nodesToAppend.push(cloneNodeKeep(extraitTitleEl));
        nodesToAppend.push(cloneNodeKeep(pickOne(".extrait")));
      ...
      case "projet7": { // Déjà demain
        nodesToAppend.push(cloneNodeKeep(pickOne(".context")));
        const credit = cloneNodeKeep(pickOne(".credit"));
        if (credit) credit.classList.add("m-credit-3x");
        nodesToAppend.push(credit);
    ...
    // Media (images)
    if (mediaSpec) {
      // clone images; remove ignored credits inside cloned media if needed
      ...
      // If we must ignore certain credit nodes, remove them from text clones is already handled; but also ensure
      // we don't accidentally include them anywhere (we only include imgs).
